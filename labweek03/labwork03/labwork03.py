class Predicate:  # 定义一个谓词类来对子句中的谓词进行存储
    element = []
    ...
    if len(new_clause) == 1:  # 查找已有的子句中是否存在与新子句互补
        for i in range(len(set_of_clause) - 1):  # set_of_clause[j]超过一个谓词的取或的子句
            if len(set_of_clause[i]) == 1 and new_clause[0].get_name() == \
                    set_of_clause[i][0].get_name() and new_clause[0].element[1:] == \
    ...
    for c in str_in:
        if c == '(':
            ...
            if alpha == beta:
                ...
                clause = clause + c
    return set_of_clause


# KB = {(GradStudent(sue),),(~GradStudent(x),Student(x)),(~Student(x),HardWorker(x)),(~HardWorker(sue),)}
#{(A(tony),),(A(mike),),(A(john),),(L(tony,rain),),(L(tony,snow),),(~A(x),S(x),C(x)),(~C(y),~L(y,rain)),(L(z,snow),~S(z)),(~L(tony,u),~L(mike,u)),(L(tony,v),L(mike,v)),(~A(w),~C(w),S(w))}
#{(On(tony,mike),),(On(mike,john),),(Green(tony),),(~Green(john),),(~On(xx,yy),~Green(xx),Green(yy))}

def main():
    set_of_clause = []
    str_in = input()
    origin_clause_set = input_clause(str_in)
    num_of_clause = len(origin_clause_set)
    for i in range(num_of_clause):
        clause_in = origin_clause_set[i]
        if clause_in == "":
            ...
            if len(set_of_clause[i]) == 1:  # 只对一个谓词的子句set_of_clause[i]进行下面处理
                for j in range(0, len(set_of_clause)):  # 和其它的子句进行比较
                    ...
                    for k in range(len(set_of_clause[j])):  # 在子句set_of_clause[j]中找相同的谓词，且可以消去，设置key为其位置
                        if set_of_clause[i][0].get_name() == set_of_clause[j][k].get_name() and \
                                set_of_clause[i][0].get_pre() != set_of_clause[j][k].get_pre():
                            key = k
                            for l in range(len(set_of_clause[j][k].element) - 1):  # 找到可以换名的变量并记录
                                if (len(set_of_clause[j][k].element[l + 1]) == 1) or (len(set_of_clause[j][k].element[l + 1]) == 2):  # 是自由变量
                    ...
                    if key == -1:  # 否则换名 消去 生成新子句
                        continue
                    new_clause = []  # 记录生成的新子句
                    for mn in range(len(set_of_clause[j])):
                        if mn != key:  # 位置为key的已经被消去了，所以不在新子句里
                            p = Predicate("")
                            p.new(set_of_clause[j][mn].element)
                            p.rename(old_name, new_name)
                            new_clause.append(p)
                    if len(new_clause) == 1:  # 判断是否生成的子句是否与已有重复（不判断是否生成了子句）
                        for nm in range(len(set_of_clause)):
                            if len(set_of_clause[k]) == 1 and new_clause[0].element == set_of_clause[nm][0].element:
                                key = -1
                                break
