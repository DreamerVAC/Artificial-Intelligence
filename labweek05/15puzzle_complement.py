    ...
    def evaluate(self, curPuzzle):
        ...
        if self.mode == 1:
            for i in range(len(curPuzzle)):
                if self.ePuzzleHash[curPuzzle[i]] != i:
                    res += 1
        # 曼哈顿距离
        elif self.mode == 2:
            """
            ...
            for i in range(len(curPuzzle)):
                if curPuzzle[i]!=0:
                    temp = curPuzzle[i]
                    res += abs(i // self.dim - self.ePuzzleHash[temp] // self.dim) + abs(i % self.dim - self.ePuzzleHash[temp] % self.dim)
        return res
    ...
    def run(self):
        while self.openList:
            ...
            if curStatus.spaceUnit // self.dim != self.dim-1:
                ...
                newStatus.spaceUnit += 1*self.dim
...
class IDA_Star:
    ...
    def evaluate(self, curPuzzle):
        ...
        # 曼哈顿距离
            ...
            for i in range(len(curPuzzle)):
                if curPuzzle[i]!=0:
                    res += abs(i // self.dim - self.ePuzzleHash[curPuzzle[i]] // self.dim) + abs(i % self.dim - self.ePuzzleHash[curPuzzle[i]] % self.dim)
        return res
...
if __name__ == "__main__":
    ...
    solution = IDA_Star(bPuzzle = bPuzzle, spaceUnit = bPuzzle.index(0), ePuzzle = ePuzzle, dim = int(math.sqrt(len(bPuzzle))), mode = 2)
    solution.run()
    end_time = time()
