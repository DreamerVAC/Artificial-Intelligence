...
class MyAI(object):
    ...
    def alpha_beta(self,depth,a,b,chessboard:ChessBoard):
        if depth>=self.max_depth:
            ...
            for cs in chess_list:#遍历所有棋子
                if depth%2==1 and cs.team==self.team:# 该层为max
                    next=chessboard.get_put_down_position(cs)# next为能走的棋子的下一步的列表
    ...
    def get_next_step(self, chessboard: ChessBoard):
        self.alpha_beta(1,-sys.maxsize,sys.maxsize,chessboard)
        return self.old_pos+self.new_pos

    @staticmethod
    def get_next_player(player):#获取下一名行动方
        if player== 'r':
            return 'b'
        else:
            return 'r'

    @staticmethod # 获取走出下一步的棋盘
    def get_tmp_chessboard(chessboard,player_chess,new_row,new_col)->ChessBoard:
        ...
        tmp_chess.row=new_row
        tmp_chess.col=new_col
