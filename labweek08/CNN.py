...
class CNN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1),  # 卷积层1
            nn.ReLU(inplace=True),  # 激活函数
            nn.MaxPool2d(kernel_size=2, stride=2),  # 最大池化层
            nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),  # 卷积层2
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1),  # 卷积层3
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        self.classifier = nn.Sequential(
            nn.Dropout(),  # Dropout层
            nn.Linear(256 * 16 * 16, 512),  # 全连接层1
            nn.ReLU(inplace=True),
            nn.Dropout(),
            nn.Linear(512, num_classes)  # 全连接层2
        )
...
def main():
    ...
    # 创建数据加载器
    dataloaders = {x: DataLoader(image_datasets[x], batch_size=32, shuffle=True, num_workers=4) for x in ['train']}
    dataloaders['test'] = DataLoader(test_dataset, batch_size=32, shuffle=False, num_workers=4)
    ...
    # 训练和评估模型的函数
    def train_model(model, criterion, optimizer, num_epochs=65):
    ...
    # 训练模型
    model, train_loss, train_acc = train_model(model, criterion, optimizer, num_epochs=65)
